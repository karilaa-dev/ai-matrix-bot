...
- A dedicated, non-admin Matrix account and its access token.
...
1. Copy [`.env.example`](./.env.example) to `.env` and set
   `MATRIX_HOMESERVER_URL`, `MATRIX_BOT_USER_ID`, `MATRIX_OWNER_ID`, and the
   stable `MATRIX_DEVICE_ID` belonging to the supplied access token.
2. Create `secrets/matrix_access_token` containing only the dedicated account's
   access token. If account recovery already exists, create
   `secrets/matrix_recovery_key` containing its recovery key. Otherwise create
   an empty placeholder and use the new-identity flow below.
3. Restrict both files: `chmod 600 secrets/matrix_access_token secrets/matrix_recovery_key`.
4. Initialize and validate the stable Matrix device. With an existing recovery
   key, run:
   ...
   If this account has no recovery identity yet, Docker's `/run/secrets` mount
   cannot receive the newly generated key because it is read-only. Create an
   empty owner-only placeholder for Compose, use a temporary writable bootstrap
   mount, then provision the generated file as the runtime secret:

   ```sh
   mkdir -p secrets/bootstrap
   chmod 700 secrets/bootstrap
   touch secrets/matrix_recovery_key
   chmod 600 secrets/matrix_access_token secrets/matrix_recovery_key
   docker compose run --rm --no-deps \
     -v "$PWD/secrets/bootstrap:/bootstrap-secrets" \
     bot npm run matrix:bootstrap -- \
     --recovery-key-out /bootstrap-secrets/matrix_recovery_key
   mv secrets/bootstrap/matrix_recovery_key secrets/matrix_recovery_key
   chmod 600 secrets/matrix_recovery_key
   ```

   Remove the empty `secrets/bootstrap` directory afterward. Bootstrap never
   stores the account password.

5. Authenticate Codex into its persistent Docker volume:
...
workspaces, Matrix sync and crypto state, Codex authentication, and the Docling
cache in separate named volumes.
...
Copy `.env.example` to `.env`, adjust it, and create the owner-only files under
`secrets/`. Export that file in your shell (`set -a; source .env; set +a`), then
use `npm run matrix:bootstrap` once and `npm run dev` to start the sync loop.
